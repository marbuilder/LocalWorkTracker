...
`s04-legacy-migration.html` und `t16-task-timer-linking.html` sind die beiden Tests, die neu für den Merge hinzugekommen sind — sie decken die Legacy-Migration bzw. die `taskId`-Verknüpfung ab, alles andere ist von LocalTasks/LocalTimetracker übernommen.
