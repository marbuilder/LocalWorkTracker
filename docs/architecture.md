...
<body>
  ...
  <div class="container">
    ...
    <div class="current-row">                  -- sticky bar, always visible on every main tab
      <div id="currentTimer">                  -- TimeModule-owned; static shell holding #currentTimerStatus (live-rendered by renderCurrentTimer()) plus the former "Timer starten" panel's controls, laid out as ONE compact flex row (`.current-controls`, mirrors `.pomodoro-controls`'s density so both boxes are the same height): #ticketNumber/#ticketNumberPreset, #timerStartTime (label dropped, `aria-label` only), #taskNotes inside a `<details class="field-info current-notes-popover" data-ui-key="timerNotes">` (reuses the `.field-info` floating-popover pattern — see "CSS" below — instead of a block `.compact-menu`, so opening it never changes the box's height), the (i) time-usage hint (also `.field-info`), and icon-only #startBtn/#stopBtn/#resetFormBtn (▶/⏹/↺, `aria-label` carries the German text) — moved here verbatim, "Timer starten" panel removed from the Zeit tab
...
The sticky bar's right column (`.current-side`, AppShell-owned) stacks `#pomodoroBar` and `#quickCapture` vertically; `#quickCapture` reuses TaskModule's `.capture-row` grid pattern (input + button) unchanged. `.current-row` uses `align-items: start` so `#currentTimer` sizes to its own content instead of stretching to match `.current-side`'s combined (Pomodoro + Schnellerfassung) height — this is what keeps the timer box visually the same height as the Pomodoro box alone, not the taller stacked pair next to it.

The timer's `#currentTimer` control row (`.current-controls`) is deliberately as dense as `.pomodoro-controls`: ticket input/preset, the `#timerStartTime` input (no visible label, `aria-label` only — the old label row was dropped to save a line), a notes icon (📝) and the existing (i) time-usage hint, and icon-only Start/Stop/Reset buttons (`.icon-btn`, `▶`/`⏹`/`↺`, `title`+`aria-label` carry the German text since there's no visible label). The notes field reuses `.field-info`'s floating-popover `<details>` pattern (the same one the (i) time-usage hint uses) — **not** `.compact-menu` — specifically so opening it never changes `#currentTimer`'s box height, which a block-level `.compact-menu` expansion would.
